...
def run_visual_test(test_case: int, simulation_duration=20) -> dict:
    ...
    pacman = Player(initial_position=pacman_spawn.rect.topleft, speed=TILE_SIZE * 4)
    pacman_group = pg.sprite.GroupSingle(pacman)

    # Tạo đối tượng path dispatcher với thuật toán UCS.
    # Chú ý: đối tượng này sẽ lưu các số liệu thật sau mỗi lần gọi UCS vào thuộc tính last_stats.
    ...
    while running:
        ...
        ghost.update(dt)  # Trong ghost.update, path_dispatcher sẽ gọi UCS và cập nhật last_stats
    ...
    # Lấy số liệu thực từ đối tượng path_dispatcher (đã được cập nhật bởi quá trình gọi UCS)
...
def main():
    ...
    for test in range(1, 6):
        print(f"\nĐang chạy Visual Test Case {test} với cấu hình spawn phức tạp...")
